...
###########################################
# 1. 通過 ImageFolder 建立數據集 (train/val)
###########################################
...
class TestDataset(Dataset):
    def __init__(self, test_dir, transform=None):
        ...
        self.image_files = [
            f for f in os.listdir(test_dir) if os.path.isfile(
                os.path.join(
                    test_dir, f)) and f.lower().endswith(
                ('.png', '.jpg', '.jpeg'))]
...
with torch.no_grad():
    for images, img_names in test_loader:
        ...
        for i, img_name in enumerate(img_names):
            ...
            predictions.append(
                (img_name, train_dataset.classes[pred_class_id]))
